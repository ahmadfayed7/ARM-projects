...
void UART_OutStringd(unsigned char buffer[]){
	int i=0 ;
	if((data_d[0] =='0')&&(data_d[1]=='0')&&(data_d[2] =='0')&&(data_d[3] =='0'))
{ //UART_OutChar(SP);
	UART_OutChar(SP);
	UART_OutChar(SP);
  UART_OutChar('0');
...
void UART_OutUDec(unsigned long n){
  ...
  UART_OutStringd(data_d);  // output using your function
}
...
void UART_OutDistance(unsigned long n){
  ...
  UART_OutString(data_s);       // output using your function
}
